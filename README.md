...
**0.2.0-alpha.4 是功能测试版。** 完整多格式阅读器仍在后续路线中。
...
默认文件留在本机；不使用遥测、远程字体或更新检查。EPUB 内脚本禁用，阅读页面的外部网络请求被阻止。用户主动使用 AI 翻译时，所选范围的文字会发送给选定服务商；安装 Docling 或 MinerU 时需要从 Python 包和模型来源下载组件。API Key 使用系统安全存储加密保存在本机。

## Windows 下载

前往 [GitHub Releases](https://github.com/Fiochanqwq/mitotoReader/releases/tag/v0.2.0-alpha.4)。推荐下载 `mitotoReader-0.2.0-alpha.4-Setup.exe`，双击安装后从桌面或开始菜单启动。无需安装开发环境；安装版阅读数据保存在 `%APPDATA%/mitotoreader/`，卸载时保留。

也可选择便携 ZIP：

解压整个发行包，双击 `mitotoReader.exe`。保留随包文件。设置写入同目录 `mitoto-data/`；请放在当前用户可写的目录。OCR 引擎、语言数据、PDF 字体资源和 Chromium 运行时都包含在包内。

`Ctrl+O` 打开文档；`PageUp/PageDown` 或 `←/→` 翻页；`Esc` 收起工具抽屉。固定版式文档保留原件字体和颜色；排版面板仅用于 EPUB。

`Ctrl+F` 搜索、`Ctrl+D` 添加 / 移除书签、`Ctrl+T` 切换快捷工具、`F11` 全屏、`F9` 切换纯净阅读、`Ctrl+＋/−` 或 `Ctrl+鼠标滚轮` 缩放、`Ctrl+0` 适应整页。纯净阅读只隐藏应用内上下栏，鼠标移至窗口上下边缘可唤出，不会自动进入系统全屏。阅读工具集中在右侧抽屉；独立工作台可处理整份 PDF OCR、使用本地解析引擎，并翻译选中内容、当前页或整份文档。`Esc` 按顺序关闭快捷键帮助、取消框选、退出纯净阅读、关闭抽屉或退出全屏。文字输入时不响应翻页、书签和缩放快捷键；底部 `?` 可查看帮助。
...
输出便携目录 `release/mitotoReader-0.2.0-alpha.4-win32-x64/`，以及 `release/installers/mitotoReader-0.2.0-alpha.4-Setup.exe`。打包后再次运行测试：

```powershell
$env:MITOTO_TEST_EXE = (Resolve-Path 'release/mitotoReader-0.2.0-alpha.4-win32-x64/mitotoReader.exe').Path
npm test
```
